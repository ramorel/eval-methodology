...
========================================================================
120 Persons (model × scaffold/persona × seed) × 75 informative Items × 4 Dimensions
...
for _, row in df.iterrows():
    ...
    response_matrix[pi, ji] = row["stereotyped"]
...
print(f" {'d':>7} {'p':>5} {'Category':<22} {'Condition':<10}")
...
for idx in sort_idx:
    ...
    print(f" {d_means[idx]:>7.2f} {raw_p:>5.2f} {cat:<22} {cond:<10}")
...
for dim in range(D):
    ...
    if strong:
        for item_id, val in strong:
            cat = item_meta.loc[item_id, "category"] if item_id in item_meta.index else ""
            cond = item_meta.loc[item_id, "context_condition"] if item_id in item_meta.index else ""
            print(f"  {item_id:>4}  a={val:>6.2f}  {cat:<22} {cond}")
    else:
        print("  (none)")
...
# ── 9. Save results ─────────────────────────────────────────────────────────
pr_df.to_csv("mirt_bbq_results.csv", index=False)
...
pd.DataFrame(item_results).to_csv("bbq_mirt_item_results.csv", index=False)

# Save losses for convergence plot
pd.DataFrame({"step": range(len(losses)), "loss": losses}).to_csv(
    "mirt_bbq_svi_losses.csv", index=False
)

print("\nSaved: mirt_bbq_results.csv, bbq_mirt_item_results.csv, mirt_bbq_svi_losses.csv")
print("Done.")